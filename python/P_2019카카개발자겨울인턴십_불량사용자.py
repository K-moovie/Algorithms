...
Author: Kim YeongHwa
Date: 2021-12-20
Title: Programmers/2019 카카오 개발자 겨울 인턴십 불량 사용자
Language: Python 3
Contents:
    문제 설명
...
def solution(user_id, banned_id):
    ...
    all_cases = list(permutations(user_id, ban_len))
    for case in all_cases:
        count = 0
        for i in range(ban_len):
            if len(case[i]) == len(banned_id[i]) and re.match(banned_id[i].replace('*', '.'), case[i]):
                count += 1
            else:
                break
        if ban_len ==  count:
