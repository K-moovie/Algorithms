...
def solution(play_time, adv_time, logs):
    answer = ''
    time_pattern = re.compile('(?P<hour>\d\d):(?P<minute>\d\d):(?P<second>\d\d)')

    def str_to_sec(string):
        temp = time_pattern.match(string)
        hour, minute, second = map(int, temp.groups())
    ...
    def sec_to_str(time):
        ...
        time %= 60
        second = str(time)
        return ':'.join(map(lambda x: x.zfill(2), [hour, minute, second]))

    play_time, adv_time = map(lambda x: str_to_sec(x), [play_time, adv_time])
    all_times = [0 for _ in range(play_time + 1)]

    for log in logs:
        log = log.split('-')
        start, end = map(lambda x: str_to_sec(x), log)
        all_times[start] += 1
        all_times[end] -= 1

    total_viewer = 0
    for index, viewer in enumerate(all_times):
        total_viewer += viewer
        all_times[index] = total_viewer

    answer_time = 0
    adv_sum = 0
    for i in all_times[:adv_time]:
        adv_sum += i
    adv_sum_max = adv_sum

    # 투포인터 알고리즘 사용
    adv_start, adv_end = 0, adv_time
    while adv_end < play_time:
        adv_start += 1
        adv_end += 1
        adv_sum = adv_sum - all_times[adv_start] + all_times[adv_end]

        if adv_sum > adv_sum_max:
            adv_sum_max = adv_sum
            answer_time = adv_start + 1
    return sec_to_str(answer_time)
